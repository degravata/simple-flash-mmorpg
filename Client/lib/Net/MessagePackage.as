package com.SS.Net 
{
	...
	public class MessagePackage
	{
		 ...
		 * @param	i_Action
		 */
		public function SetPackageAction(i_usPackageAction):void {
		 ...
		 * @param	i_intData
		 */
		public function AddMsgDataNumber(i_usAction, i_intData):void {
			if (i_intData>=-128 && i_intData<=127){	//-128~127之间
				AddMsgDataChar(i_usAction,i_intData);
			}
			else if(i_intData>=128 && i_intData<=255){	//128~255之间
				AddMsgDataUChar(i_usAction,i_intData);
			}
			else if(i_intData>=-32768 && i_intData<=32767){	//–32768~32767
				AddMsgDataShort(i_usAction,i_intData);
			}
			else if(i_intData>=32768 && i_intData<=65535){	//32768~65535
				AddMsgDataUShort(i_usAction,i_intData);
			}
				...
				AddMsgDataUInt(i_usAction,i_intData);
			}
			else {
				AddMsgDataInt(i_usAction,i_intData);
			}
		...
		public function AddMsgDataUInt(i_usAction, i_uintData:uint):void {
			var ba:ByteArray = new ByteArray();
			ba.writeUnsignedInt(i_uintData);
			ba.position = 0;
			AddMsgDataByte(i_usAction, ba);
		}
		public function AddMsgDataInt(i_usAction, i_intData:int):void {
			var ba:ByteArray = new ByteArray();
			ba.writeInt(i_intData);
			ba.position = 0;
			AddMsgDataByte(i_usAction, ba);
		}
		public function AddMsgDataUShort(i_usAction, i_ushortData:uint):void {
			var ba:ByteArray = new ByteArray();
			ba.writeBytes(UShortToBytes(i_ushortData));
			ba.position = 0;
			AddMsgDataByte(i_usAction, ba);
		}
		public function AddMsgDataShort(i_usAction, i_shortData:int):void {
			var ba:ByteArray = new ByteArray();
			ba.writeBytes(ShortToBytes(i_shortData));
			ba.position = 0;
			AddMsgDataByte(i_usAction, ba);
		}
		public function AddMsgDataUChar(i_usAction, i_ubyteData:uint):void {
			var ba:ByteArray = new ByteArray();
			ba.writeByte(NubmerToUByte(i_ubyteData));
			ba.position = 0;
			AddMsgDataByte(i_usAction, ba);
		}
		public function AddMsgDataChar(i_usAction, i_byteData:int):void {
			var ba:ByteArray = new ByteArray();
		...
		public function AddMsgDataByte(i_Action, i_byteArrContent:ByteArray):Boolean {
