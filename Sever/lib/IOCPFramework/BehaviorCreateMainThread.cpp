		...
		namespace IOCPFramework{
			namespace Behavior{
				...
				BehaviorCreateMainThread::~BehaviorCreateMainThread(void)
				{
					...
					if (NULL!=this->_lpMainListenIOCPKey){
				...
				DWORD BehaviorCreateMainThread::CompletionRoutine(LPVOID lp_param){
					...
					while(1)
					{
						...
						if(lp_io->operation==0){

						}else if(lp_io->operation==1){
							//lp_this->AddMessage(lp_io->buf);
						}else if(lp_io->operation==2){
						...
						case IOCP_WRITE:
						{
							//lp_this->_lpBaseBehaviorMsgDeal->RemoveSendObj(sendObj);
